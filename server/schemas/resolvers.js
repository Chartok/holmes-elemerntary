...
//                 password: encryptedPassword,
//             });
//             const token = jwt.sign({ user_id: newUser._id, email }, 'dontputthesecrettokenhereinquotes', { expiresIn: '2h' });
//             newUser.token = token;
//             const res = await newUser.save();
//             return { id: res.id, ...res._doc };
//         },
...
//             const user = await User.findOne({ email });
//             if (user && (await bcrypt.compare(password, user.password))) {
//                 const token = jwt.sign({ user_id: user._id, email }, 'dontputthesecrettokenhereinquotes', { expiresIn: '2h' });
//                 user.token = token;
//                 return { id: user.id, ...user._doc };
//             }
//             else {
...
//             if (context.user) {
//                 try {
//                     const updatedUser = await User.findByIdAndUpdate(context.user._id, { $addToSet: { savedBooks: book } }, { new: true });
//                     return updatedUser;
//                 }
//                 catch (err) {
//                     throw new Error('Error saving book!');
...
//         },
//         removeBook: async (_, { userId, bookId }) => {
//             const updatedUser = await User.findOneAndUpdate({ _id: userId }, { $pull: { savedBooks: { bookId } } }, { new: true });
//             if (!updatedUser) {
//                 throw new Error('Could not remove book!');
//             }
//             return updatedUser;
